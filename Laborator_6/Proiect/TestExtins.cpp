...
void testAdauga() {
	...
	assert(c.vida() == false);
	assert(c.dim() == 40);
	for (int i = -100; i < 100; i++) { //mai adaugam elemente [-100, 100), acum anumite elemente [0, 10) sunt de 3 ori, allele [-10, 0), si [10, 20) sunt de 2 ori
		c.adauga(i);		
	}
	assert(c.vida() == false);
	assert(c.dim() == 240);
...
void testQuantity() {//scopul e sa adaugam multe date 
    ...
    //assert(c.dim() == 0) aici da -6, probabil e ceva neinregula la adaugare sau stergere..
}
