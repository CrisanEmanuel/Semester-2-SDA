...
void testSterge() {
	...
	for (int i = 100; i > -100; i--) { //stergem descrescator (in ordine inversa fata de ordinea adaugarii)
		if (i % 2 == 0) {
			assert(c2.sterge(i) == true);
		}
		else {
			assert(c2.sterge(i) == false);
		}
	}
	...
	for (int i = -30; i < 30; i++) { //stergem o parte dintre elemente
		...
		assert(c4.cauta(i) == true);
		assert(c4.sterge(i) == true);
		if (i != 0) {
			assert(c4.cauta(i) == false);
		}
		else {
			assert(c4.cauta(i) == true);
		}
...
void testIterator() { // nu stim reprezentarea Colectiei, putem testa doar anumite lucruri generale, nu stim in ce ordine vor fi afisate elementele.
	...
	assert(elemente2.size() == c5.dim());
 	for (int firstElem : elemente2) { //scoatem pe rand elemente din vectorul dat de iterator, verificam sa fie in colectie si le stergem si din vector si din c4. Incepem stergerea cu primul
